...
class Tasker(commands.Cog):
    def __init__(self, bot: IBot) -> None:
        ...
        self.task_functions = {
            "Tuesday": self.tuesday_task,
            "Friday": self.friday_task,
            "Sunday": self.sunday_task,
        }
        self.once = False

    def get_avail_message(self, role: Role):
        return (
            f"{role.mention} please click {self.bot.get_command_mention(role.guild.id, 'submitavailability')} "
            "to submit your availability. Remember you must provide your owner a minimum "
            "of four days each week to be considered active."
        )
    ...
    async def tuesday_task(self, simulation: bool = False):
    ...
    def _day_task(self, day: Days, hour: int = 17):
        print("[+] Crating task for", day)
        now = datetime.datetime.utcnow()
        date = get_next_date(day=day, hour=hour)
